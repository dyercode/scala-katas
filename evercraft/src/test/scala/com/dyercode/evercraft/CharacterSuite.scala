...
import org.scalatest.{MustMatchers, OneInstancePerTest, BeforeAndAfter, FunSuite}

class CharacterSuite extends FunSuite with MustMatchers with BeforeAndAfter with OneInstancePerTest {
  ...
  test("character hitPoints must increase by 5 plus con mod for each level") {
    ...
    character.hitPoints must be (startingHP + 5 + character.constitution.modifier)
  }
