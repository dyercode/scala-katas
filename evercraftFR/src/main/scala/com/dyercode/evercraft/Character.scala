...
case class Character(
  ...
  def constitutionModifier: Int =
    constitution.modifier + race.constitutionModifier
  def intelligenceModifier: Int =
    intelligence.modifier + race.intelligenceModifier
...
given Combatant[Character] with {
  ...
  extension [A: Raced](defender: Character)
    def armorClass(attacker: A): Int = 10
      + acDexBonus(defender)
      + defender.playerClass.acMod(defender)
      + defender.race.acMod(attacker)

  extension [B: Combatant: Aligned: Raced](attacker: Character)
    def attack(roll: Int, defender: B): AttackResult = {
      val critRange = 20 - attacker.race.critRangeModifier
      if (roll >= critRange) AttackResult.Crit
      else {
        val targetAc =
          defender.armorClass(attacker) + attacker.playerClass.targetAcModifier(
            defender
          )
  ...
  extension [D: Aligned: Raced](c: Character)
    def calculateDamage(ar: AttackResult, defender: D): Int = {
      val rawDamage = c.playerClass.baseDamage
        + c.strengthModifier
        + c.playerClass.extraDamage(defender)
        + c.race.extraDamage(defender)
      val critMultiplier =
        if (ar == AttackResult.Crit) c.playerClass.critMultiplier(defender)
        else 1
  ...
  extension (c: Character)
    def hitPoints: Int = {
      val cap = Math.max(
        1,
        c.playerClass.baseHitPoints + c.constitutionModifier + c.race
          .hitPointModifier(c)
      ) * c.level
      cap - c.damage
    }
  ...
  extension (c: Character)
    def takeDamage(dmg: Int): Character =
      c.copy(damage = c.damage + dmg)

  extension (c: Character) def dead: Boolean = hitPoints(c) <= 0

  extension [B: Combatant: Aligned: Raced](character: Character)
    def attackBonus(defender: B): Int = {
